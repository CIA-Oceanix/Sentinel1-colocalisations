...
from check_args import SATELLITE_PLATFORMS
...
def reproject(platform, data, platform_lat, platform_lon, owi_lat, owi_lon):
    if platform in SATELLITE_PLATFORMS['any'] or platform == 'ERA':
...
def generate_gif(iw_polygon, channel, urls_per_platforms, gif_filename, verbose, read_function, download=True, delta_factor=None):
    ...
    log_print(f"Generate .png", 2, verbose)
    for platform in filenames_per_platform:
        ...
        for date, filenames in filenames_per_platform[platform].items():
            ...
            if platform in SATELLITE_PLATFORMS['any'] or platform == 'ERA5':
